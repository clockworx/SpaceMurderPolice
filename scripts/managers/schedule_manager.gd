...
func _ready():
    ...
    # Enable schedules for all NPCs by default after a short delay
    call_deferred("_enable_all_npc_schedules")
...
func _initialize_default_schedules():
    # Dr. Marcus Webb - Chief Scientist schedule
    var webb_schedule = [
    ...
    npc_schedules["Dr. Marcus Webb"] = webb_schedule
...
func _update_all_npc_schedules():
    # Only update NPCs that actually exist in the scene
    var npcs = get_tree().get_nodes_in_group("npcs")
    
    for npc in npcs:
        if npc is NPCBase and npc_schedules.has(npc.npc_name):
            var scheduled_room = get_npc_scheduled_room(npc.npc_name)
            schedule_changed.emit(npc.npc_name, scheduled_room)
...
func update_npc_schedule(npc_name: String):
    """Force update schedule for a specific NPC"""
    if npc_schedules.has(npc_name):
        var scheduled_room = get_npc_scheduled_room(npc_name)
        schedule_changed.emit(npc_name, scheduled_room)

func _enable_all_npc_schedules():
    """Enable schedules for all NPCs in the scene"""
    var npcs = get_tree().get_nodes_in_group("npcs")
    for npc in npcs:
        if npc is NPCBase:
