...
func _ready():
	...
	# Wait a bit for spawn manager to create evidence
...
func _find_evidence_recursive(node: Node):
	if node.get_class() == "StaticBody3D" and node.has_method("get_evidence_data"):
		if node.has_signal("evidence_collected"):
...
func get_evidence_by_name(name: String) -> Dictionary:
...
func has_evidence(name: String) -> bool:
	return get_evidence_by_name(name).size() > 0
